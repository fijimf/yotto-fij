...
public class StatisticsTimeSeriesService {
    ...
    /**
     * Population-stat names owned by this service (z-score inputs for the wide
     * snapshot). season_population_stats deletes are scoped by stat name and the
     * table is shared with TeamStatTimeSeriesService's calculators — every other
     * writer MUST use disjoint names (guarded by StatCatalogTest).
     */
    static final List<String> STAT_NAMES =
    ...
    public void calculateAndStoreForSeason(SeasonGameData data, LocalDate fromDate) {
        ...
        Map<Long, List<RpiCalculator.GameRecord>> gamesList = new HashMap<>();
        ...
        for (Map.Entry<LocalDate, List<Game>> entry : gamesByDate.entrySet()) {
            ...
            for (Game game : entry.getValue()) {
                ...
                RpiCalculator.addGame(gamesList, homeId, awayId, neutral, homeWon);
            }
            ...
            Map<Long, RpiCalculator.RpiComponents> rpiByTeam = RpiCalculator.compute(gamesList);
            for (TeamSeasonStatSnapshot snap : dateSnaps) {
                RpiCalculator.RpiComponents rpiC = rpiByTeam.get(snap.getTeam().getId());
                if (rpiC != null) {
