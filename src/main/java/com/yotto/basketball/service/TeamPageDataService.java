...
import com.yotto.basketball.controller.TournamentBadgeFormatter;
...
/**
 * Builds the team page's season schedule and stat panel — extracted from
 * TeamWebController so both can be cached per (team, season). Load testing
 * measured ~700ms of DB work per uncached team-page view, which collapsed the
 * shared connection pool at 10 concurrent requests. Everything returned is a
 * plain record tree (no JPA entities), evicted post-scrape via
 * {@link PageCacheEvictionService}.
 */
@Service
public class TeamPageDataService {
    ...
    public TeamPageDataService(SeasonRepository seasonRepository,
    ...
    /** Full schedule + records for one team-season. Throws when the season year is unknown. */
    @Cacheable(value = CacheConfig.TEAM_SCHEDULE, key = "#teamId + ':' + #year")
    @Transactional(readOnly = true)
    public SeasonSchedule buildSeasonSchedule(Long teamId, int year) {
        Season season = seasonRepository.findByYear(year)
                .orElseThrow(() -> new EntityNotFoundException("Season not found: " + year));

        SeasonStatistics ss = seasonStatisticsRepository.findByTeamIdAndSeasonId(teamId, season.getId()).orElse(null);
        List<Game> games = gameRepository.findByTeamAndSeasonWithDetails(teamId, season.getId());
        ...
        return new SeasonSchedule(
                ...
                games.stream().map(g -> toGameRow(g, teamId)).toList(),
                computeRegularSeasonRecord(games, teamId),
                computeTournamentRecords(games, teamId)
        );
    }
     ...
     */
    @Cacheable(value = CacheConfig.TEAM_STAT_PANEL, key = "#teamId + ':' + #year")
    @Transactional(readOnly = true)
    public TeamStatPanel buildStatPanel(Long teamId, int year) {
        Season season = seasonRepository.findByYear(year)
                .orElseThrow(() -> new EntityNotFoundException("Season not found: " + year));
    ...
    public record StatRow(
        ...
        public boolean linkable() {
            return StatCatalog.contains(statName);
        }
