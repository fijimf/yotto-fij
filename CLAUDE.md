...
- **Snapshot entities** - TeamSeasonStatSnapshot (wide per-team daily stats), TeamPowerRatingSnapshot + PowerModelParamSnapshot (power models: MASSEY, MASSEY_TOTALS, BRADLEY_TERRY, BRADLEY_TERRY_W, plus ADJ_OFF/ADJ_DEF/ADJ_TEMPO per-possession adjusted efficiencies + tempo from AdjustedEfficiencyRatingService — needs team_game_stats; λ via `app.ratings.adj-efficiency.lambda`), SeasonPopulationStat (long-format population distributions, shared across services via stat-name-scoped deletes), TeamStatSnapshot (long-format derived stats, e.g. four factors — new stats are registry entries in BoxScoreStatCalculator, not migrations)
...
- **Neutral-site ML symmetry** - `PredictionService` scores every ML bundle on both listings of a neutral-site game (home/away swapped via `GameRatings.mirrored()`) and averages, so predictions are invariant to which team ESPN lists as home. Needed because the training data's neutral "home" is usually the better seed and the bundles learned a ~4-pt listing bump (Sept 2026 pairwise audit). Classical models are symmetric by construction
- **PredictionEvaluation** - one row per FINAL game × model (MASSEY/MASSEY_TOTALS/BRADLEY_TERRY/BRADLEY_TERRY_W/ADJ_EFF/ML:&lt;slug&gt;/BOOK benchmark): pre-game prediction + error vs. actual, built retroactively from snapshot time series (leakage-free) by PredictionEvaluationService — runs incrementally after scrapes and on demand from /admin; powers the public `/models/compare` page and the per-model hub pages. Note: `betting_odds.spread` is handicap orientation (negative = home favored)
...
## Public information architecture (2026-08 menu reorganization)

Spec + plan: docs/MENU_AND_GUI_SPEC.md, docs/MENU_AND_GUI_IMPLEMENTATION_PLAN.md. Six nav sections with one-level dropdowns (deeper nesting = tab strips on the page, never nested flyouts):
- **Games** ▾ `/games`, `/bracket` (always visible; year tabs on the page)
- **Statistics** ▾ `/seasons/{y}/stats` (League Overview), `/stats/predictor[/{stat}]` (AUC ranking + red/green scatter), `/stats/correlation` (2–8 var picker, URL state + `stats.correlation-vars` pref), 8 category pages `/stats/{results|scoring|efficiency|four-factors|shooting|rebounding|playmaking|defense}`, `/stats` glossary. Results/Scoring stats (`wp/owp/oowp/ppg/opp_ppg/scoring_margin/margin_volatility`) come from `ResultsStatCalculator` (game results only, RPI components via shared `RpiCalculator`) — their names MUST stay disjoint from `StatisticsTimeSeriesService.STAT_NAMES` (stat-name-scoped pop-stat deletes; guarded by StatCatalogTest)
- **Power Rankings** ▾ `/rankings` (composite table only — Model View/Scatter Matrix tabs retired) + `/rankings/{rpi|massey|bradley-terry|bradley-terry-weighted|adjusted-efficiency}` per-model pages (PowerRankingPageService)
- **Models** ▾ `/models` index, `/models/compare` (was `/predictions/performance`), `/models/matchup` (was `/predictions/matchup`), plus one hub per public model `/models/{slug}` (About | Schedule | Bracket | Matchup tabs). Public = ACTIVE+loaded ML bundles + classical `adjusted-efficiency` (PublicModelService; NavModelsAdvice fills the dropdown from the cached serving plan). `/predictions` 301s to the default model's schedule
- Legacy URLs 301 via LegacyRedirectController. `currentSection` model attribute drives nav active state. Shared fragments + `--chart-*`/`chart-theme.js` rules: see UI.md

## Caching (2026-08, after load testing)

Three layers, all serving anonymous-identical data that changes only after scrapes:
- **nginx `proxy_cache`** (config/mysite, zone `deepfij`, 60s TTL + `proxy_cache_lock` + serve-stale) for anonymous GETs; bypassed on session/remember-me cookies and auth/admin paths. Plus gzip. Invariants in the `server-ops` skill.
- **In-app Caffeine caches** (`CacheConfig`, 6h TTL backstop): `ModelComparePageService` (extracted from ModelPerformanceController), `ModelAboutService.build/headlines`, `StatPageService.build`, `TeamPageDataService.buildSeasonSchedule/buildStatPanel` (extracted from TeamWebController — the records templates use live there now). Values may hold repo projection proxies — in-process only, never a serializing store. **Eviction**: `PageCacheEvictionService.evictAll()` (also clears SeasonWrap + StatPredictiveness hand-rolled caches) fires at the end of ScrapeOrchestrator.runCalculations, AsyncScrapeService.evaluatePredictionsAsync, and admin ml/reload. New expensive read paths should join this pattern, and `BaseIntegrationTest.wipeDatabase()` already evicts between tests (RESTART IDENTITY reuses entity IDs, so stale cache entries would cross tests).
- **Static assets**: content-hashed URLs + 7d browser cache (`spring.web.resources.*`; overrides Security's no-store for those paths). Disabled in the dev profile for hot-reload. `SET statement_timeout = '30s'` on every pooled connection kills client-abandoned queries — keep individual statements (and Flyway migrations) under it.
...
- OFFSEASON "Hits and Misses" panel: one game a day from a top-50 pool built from `prediction_evaluations` for the site's default model vs BOOK (`PredictionEvaluationRepository.findModelHitsBookMisses` / `findSharedMisses`): weekdays a hit (model within 1.5 pts, book off by the most), Sundays a miss (model and book both badly wrong). Pick is seeded by the date so the nginx cache and the digest agree; pools span all seasons, including the ML model's in-sample ones
